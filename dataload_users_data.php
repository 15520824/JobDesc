...
include_once "connection.php";
    ...
    $account = $connection->load($prefixCompany."users","id=".$_POST["id"]);
    
    if($account[0]["privilege"]==1||$account[0]["privilege"]==2){
        $data = $connection->load($prefixCompany."datauser");  
        $data = $data;
    }
    else
    $data = $connection->load($prefixCompany."datauser","userid=".$_POST["id"]);
