<?php
    function write_reporter_departments_script(){
?>
<script type="text/javascript">
"use strict";

jobdesc.reporter_departments.generateData = function(host) {
    var data, header;
    DOMElement.removeAllChildren(host.category_container);
};

jobdesc.reporter_departments.init = function(host, mode) {
    host.database = {};
    host.root_category = 0;
    DOMElement.removeAllChildren(host.pagetitle);
    host.pagetitle.appendChild(DOMElement.textNode(LanguageModule.text("title_organizational_chart")));
    host.holder.titleText = LanguageModule.text("title_organizational_chart");
    ...
    Promise.all(promiseAll).then(function() {
                                ...
                                on: {
                                    click: function(host) {
                                        return function() {
                                            ...
                                            //     index: 1,
                                            //     title: "Thêm chỉ mục",
                                            //     bodycontent:blackTheme.reporter_departments.addPosition()
                                            // });
                                            host.frameList.add(
                                                blackTheme
        ...
        switch (mode) {
            case 1:
                jobdesc.reporter_departments_information.init(container, host)
                break;
            case 2:
